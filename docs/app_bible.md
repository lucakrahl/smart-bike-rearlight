...
**Version 0.22 · Stand 07.08.2026 · Status: aktiv gepflegt (Single Source of Truth der App)**

> Version 0.22 zieht die abgeschlossene **Frame-Schema-v3-Migration** (AP0–AP8) vollständig nach (freigegeben 07.08.2026, ersetzt v0.21). Das Dokument ist **selbsttragend**.
>
...
- **AP2b — Golden-Vektor-Kreuztest (E-3):** Der Produktions-Decoder dekodiert einen von der Firmware bereitgestellten 113-Byte-Golden-Vektor (`testdata/frame_v3_golden.hex/.md`) und prüft **jedes** Feld gegen die Firmware-Wertetabelle — **nicht** über den eigenen Encoder. Bestätigt: App-Decoder und Firmware-Encoder meinen dieselbe Bytebelegung (43 Felder getroffen).
...
- **AP6 — Aufzeichnungsrate (E-2):** umschaltbarer `RecordingMode` **1 Hz (Default) / 10 Hz (Validierung)**; Umschalten nur außerhalb einer Aufzeichnung; **Batch-Persistenz** (~1 Save/s statt 1 Save/Sample) über den Hintergrund-ModelActor; Skalen-Test 6000 Samples (10 min) ohne Verlust, Zeitstempel eindeutig.
- **AP7 — CSV 35 Spalten:** `temperature_c` aus der CSV entfernt (E-4), 13 v3-Felder in Offset-Reihenfolge angehängt; Präambel-`schema_version` = **Geräte-Frame-Version** (bei gemischten Versionen Minimum + `frame_version_gemischt`, E-2); `t_s` ≥ 2 Nachkommastellen (E-4); Header per Golden-Test eingefroren. Format-Referenz: `docs/CSV_Format_v3_Validierungsexport.md`.
- **AP8 — Diagnoseansicht:** read-only Ansicht unter Settings mit v3-Status (`bias_calibrated`, `gnss_accel_valid`), Zeitbudget (`dt_max_ms`/`loop_max_us`) und Defekt-Indikator `truncatedV3FrameCount` (E-1). Nichts davon im Cockpit, nicht-modal (AR-UX-01). Am realen iPhone verifiziert (u. a. `truncatedV3FrameCount == 0`, dt_max 10 ms, loop_max 53 µs).
...
- `version ≥ 3 & 81 ≤ len < 113` (kurzes v3-Frame, **E-1**) → v2-Ebene lesen (v3 = nil), **kein** Fehler, `truncatedV3FrameCount +1` (Defekt-Indikator: abgeschnittene Notification/MTU).
...
| **E-1: kurzes v3-Frame lesen + `truncatedV3FrameCount` (getrennt vom Fehlerzähler)** | ein v3-meldendes, zu kurzes Frame ist defekt (abgeschnitten/MTU); Verwerfen vernichtet gültige v2-Nutzdaten, stilles Durchwinken versteckt den Defekt | verwerfen / stumm durchwinken |
| **E-3: Golden-Vektor-Kreuztest gegen Firmware (nicht über eigenen Encoder)** | einziger Test, der die Schnittstelle selbst prüft; schließt gemeinsamen Denkfehler zweier symmetrischer Suiten aus | nur App-interner Round-Trip |
...
| **E-2: 10-Hz-Validierungsmodus (Default 1 Hz), Batch-Persistenz** | Bremsdynamik 0,3–3 s braucht Sub-Sekunden-Auflösung; 1 Hz zu grob; Batching hält AR-NFR-PERF-01 | dauerhaft 10 Hz / Save-pro-Sample |
| **`temperature_c` in Persistenz aufnehmen, nur aus CSV entfernen (E-4)** | additiv/nicht-destruktiv; „speichere, was ankommt, entscheide Export getrennt“; E-4 betrifft nur den Export | aus Persistenz weglassen / im CSV behalten (leer) |
| **E-2: CSV-`schema_version` = Minimum bei gemischten Versionen + `frame_version_gemischt`** | der schwächste Fall bestimmt, welche Spalten durchgängig gefüllt sind | Maximum (verspricht leere Spalten) |
| **E-4: `t_s` ≥ 2 Nachkommastellen** | bei 10 Hz sonst doppelte Zeitstempel → zeitbasierte Auswertung unbrauchbar | eine Nachkommastelle |
